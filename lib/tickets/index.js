  ...
  */
  ...
  async _buildSearchQueryString(options) {
    ...
    if (options) {
      ...
      props.forEach(prop => this._append(prop, options, builder))

      builder.sortBy = options.sortBy || 'lastDateUpdate'
      builder.sortDirection = options.sortDirection || 'desc'

      if (options.outputDataSet) {
        builder.outputDataSet = options.outputDataSet.join(',')
      }
